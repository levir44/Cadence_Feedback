...
test_HR_up = test_HR_up - mean(test_HR_up);

% subtract my the mean
test_SPM_up = interp1(SPM_time_up,SPM_up,time_up);
test_SPM_up = test_SPM_up - mean(test_SPM_up);
...
title('Normalized Cross-correlation between HR and CAD For Up Ramp');
...
test_HR_stay = test_HR_stay - mean(test_HR_stay);

% subtract my the mean
test_SPM_stay = interp1(SPM_time_stay,SPM_stay,time_stay);
test_SPM_stay = test_SPM_stay - mean(test_SPM_stay);
...
xlim([0 60]);
ylabel('Cross-correlation');
title('Normalized Cross-correlation between HR and CAD For Flat');
...
test_HR_down = test_HR_down - mean(test_HR_down);

% subtract my the mean
test_SPM_down = interp1(SPM_time_down,SPM_down,time_down);
test_SPM_down = test_SPM_down - mean(test_SPM_down);
...
xlim([0 60]);
ylabel('Cross-correlation');
title('Normalized Cross-correlation between HR and CAD For Down Ramp');
