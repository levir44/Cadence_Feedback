% TEST 4,5,6
% test 4 is not good, bad readings
% HR expected is at a weird in-between value of walking and jogging

load("T4.mat");
% desrired HR = 150
% length of workout = 10 mins
HR_exp = [160 160];
HR_exp_time = [T4_HR(2)/60000 T4_HR(end-2)/60000];

% everyhting worked, nothing got disconnected
% begin getting the data

% load in T1.mat which contains
% 1. T4_HR - inst. HR with time stamps
% 2. T4_SPM - inst. cadence with time stamps
% 3. T4_Cad - desired cadence
HR_time = T4_HR(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = T4_HR(1:2:end); % is in bpm

SPM_time = T4_SPM(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = T4_SPM(1:2:end); % is in steps per min (SPM)

CAD_time = T4_Cad(2:2:end);
