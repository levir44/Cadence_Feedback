% TEST 6
% test 6 is good, 150 expected
% this places the cadence at a nice fast walk instead of needing to go
...
HR_exp = [150 150];
HR_exp_time = [T6_HR(2)/60000 T6_HR(end-2)/60000];

% everyhting worked, nothing got disconnected
% begin getting the data

% load in T1.mat which contains
% 1. T6_HR - inst. HR with time stamps
% 2. T6_SPM - inst. cadence with time stamps
% 3. T6_Cad - desired cadence
HR_time = T6_HR(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = T6_HR(1:2:end); % is in bpm

SPM_time = T6_SPM(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = T6_SPM(1:2:end); % is in steps per min (SPM)

CAD_time = T6_Cad(2:2:end);
