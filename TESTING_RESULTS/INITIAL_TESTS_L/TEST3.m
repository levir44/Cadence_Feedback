...
% done at home, mostly running in place so its a little different
% trying to see what hardware is failing

load("T3.mat");
% desrired HR = 160
% length of workout = 8 mins
HR_exp = [160 160];
HR_exp_time = [T3_HR(2)/60000 T3_HR(end-2)/60000];

% everyhting worked, nothing got disconnected
% begin getting the data

% load in T1.mat which contains
% 1. T3_HR - inst. HR with time stamps
% 2. T3_SPM - inst. cadence with time stamps
% 3. T3_Cad - desired cadence
HR_time = T3_HR(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = T3_HR(1:2:end); % is in bpm

SPM_time = T3_SPM(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = T3_SPM(1:2:end); % is in steps per min (SPM)

CAD_time = T3_Cad(2:2:end);
