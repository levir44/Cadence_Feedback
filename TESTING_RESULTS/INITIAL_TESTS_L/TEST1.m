% FIRST TEST
% done in my aprtment by running back and forth in my living room and
% kitchen like a gerbel
% but it was easy to set up a test

load("T1.mat");
% desrired HR = 150
% length of workout = 10 mins
HR_exp = [150 150];
HR_exp_time = [T1_HR(2)/60000 T1_HR(end-2)/60000];

% everyhting worked, nothing got disconnected
% begin getting the data

% load in T1.mat which contains
% 1. T1_HR - inst. HR with time stamps
% 2. T1_SPM - inst. cadence with time stamps
% 3. T1_Cad - desired cadence
HR_time = T1_HR(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = T1_HR(1:2:end); % is in bpm

SPM_time = T1_SPM(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = T1_SPM(1:2:end); % is in steps per min (SPM)

CAD_time = T1_Cad(2:2:end);
