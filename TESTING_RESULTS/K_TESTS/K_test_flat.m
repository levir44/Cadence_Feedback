% Katie test 1 - on flat surface
% system did not respond well due to bio differences and running form
% differences, made the measurements extremtly noisy


load("K2.mat");
% desrired HR = 150
% length of workout = 10 mins
HR_exp = KHRE2(1:2:end);
HR_exp_time = KHRE2(2:2:end);
...
HR_time = KHR2(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = KHR2(1:2:end); % is in bpm

SPM_time = KSPM2(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = KSPM2(1:2:end); % is in steps per min (SPM)

CAD_time = KCAD2(2:2:end);
