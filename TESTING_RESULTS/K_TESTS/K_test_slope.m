% Katie test 3 - on slope
% system didnt work super well bc of noisy measurments
% didnt strike heel running up the slope so no measurements were registered

load("K3.mat");
% desrired HR = 150
% length of workout = 10 mins
HR_exp = KHRE3(1:2:end);
HR_exp_time = KHRE3(2:2:end);
...
HR_time = KHR3(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = KHR3(1:2:end); % is in bpm

SPM_time = KSPM3(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = KSPM3(1:2:end); % is in steps per min (SPM)

CAD_time = KCAD3(2:2:end);
