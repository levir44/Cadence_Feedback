% second P test to see if behavior has changed at all
% still too difficult to listen to

load("P2.mat");
% desrired HR = 150
% length of workout = 10 mins
HR_exp = PHRE2(1:2:end);
HR_exp_time = PHRE2(2:2:end);
...
% 1. PHR2 - inst. HR with time stamps
% 2. PSPM2 - inst. cadence with time stamps
% 3. PCAD2 - desired cadence
HR_time = PHR2(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = PHR2(1:2:end); % is in bpm

SPM_time = PSPM2(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = PSPM2(1:2:end); % is in steps per min (SPM)

CAD_time = PCAD2(2:2:end);
