% Second TEST AFTER UPDATING HARDWARE
% This test is a step from 150bpm to 180bpm
% using CURRENT SPM, not PREV
% Tau = 10s

% using current cadence does not perform as well as using previous cadence,
% previous better representes plant characteristics

load("U2.mat");
% desrired HR = 150
% length of workout = 10 mins
HR_exp = UHRE2(1:2:end);
HR_exp_time = UHRE2(2:2:end);
...
HR_time = UHR2(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = UHR2(1:2:end); % is in bpm

SPM_time = USPM2(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = USPM2(1:2:end); % is in steps per min (SPM)

CAD_time = UCAD2(2:2:end);
