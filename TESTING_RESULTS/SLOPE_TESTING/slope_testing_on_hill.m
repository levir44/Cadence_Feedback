...
% Tau = 10s
% running on street, up hill, down hill, back to street

load("Slope2.mat");
% desrired HR = 150
% length of workout = 10 mins
HR_exp = ZSHRE2(1:2:end);
HR_exp_time = ZSHRE2(2:2:end);
...
HR_time = ZSHR2(2:2:end);
HR_time = HR_time / 60000; % converting to mins
HR = ZSHR2(1:2:end); % is in bpm

SPM_time = ZSSPM2(2:2:end);
SPM_time = SPM_time / 60000; % also converting to mins
SPM = ZSSPM2(1:2:end); % is in steps per min (SPM)

CAD_time = ZSCAD2(2:2:end);
